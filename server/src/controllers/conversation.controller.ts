...
import { PopulatedUser, PopulatedMessage, ConversationDocument, MessageDocument } from '../utils/types/conversation.type';
...
export const getChatList = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    for (const friendship of friendships) {
      ...
      const hasConversation = existingConversations.some(conv => 
        conv.participants.some(p => 
          p._id.toString() === friend._id.toString()
        )
      );
      ...
      if (!hasConversation) {
        friendsWithoutConversations.push({
          ...
          timestamp: new Date(friendship.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    const chatList = [...formattedConversations, ...friendsWithoutConversations]
      .sort((a, b) => {
...
export const getMessages = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const formattedMessages = messages.map(message => ({
      id: message._id,
      text: message.text,
      sender: message.sender._id.toString() === userId.toString() ? 'me' : 'other',
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    res.status(201).json(
      new ApiResponse(
        ...
        isNewConversation ? "New conversation started" : "Message sent successfully"
      )
    );
  }
);